...
exports.signup = (req, res) => {
	...
	tblusers
		.create({
			...
			lastname: lastname,
			name: capitalize(firstname.toLowerCase())
				.concat(" ", capitalize(middlename.toLowerCase()))
				.concat(" ", capitalize(lastname.toLowerCase())),
			...
			tbluser_profiles
				...
				.then(() => {
					mail.transport.sendMail(
						mail.CREATE_USER_Mail(user.email, user.name, tempPass),
						(error, info) => {
							if (error) {
								return res.json({
									message:
										"Fail to send email to " +
										user.email +
										" but successful registered, Passcode: " +
										tempPass,
									password: tempPass,
								});
							}
...
exports.forgotPassword = (req, res) => {
	...
	User.findOne({
		...
		.then((result) => {
			...
			mail.transport.sendMail(
				mail.PASSWORD_RESET_Mail(result.email, result.fullname, NewPass),
				(error, info) => {
					if (error) {
						return res.json({
							message: error,
